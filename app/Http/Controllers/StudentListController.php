...
class StudentListController extends Controller
{
    public function index()
    {
        if (Auth::user()->roles->every(fn($role) => $role["role_id"] == 1)) {
            ...
            $students = Student::whereHas("program.institute", function (
                $query
            ) {
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()
            ->back()
            ->with("success", "Edited Student Successfully");
    }

    public function destroy($id)
    {
        Student::destroy($id);
        return redirect(route("student_list.index"))->with(
            "success",
    ...
    public function show($id)
    {
        ...
        $attendace_fees = AttendanceFee::whereHas("attendanceRecord", function (
            $query
        ) use ($id) {
        ...
        $payment_submissions = PaymentSubmission::whereHas("fee", function (
            $query
        ) use ($id) {
            $query->where("student_id", $id);
        })->get();
        ...
        // dd($student);
        return view(
            "student_list.actions.view_student",
            compact(
                "student",
                "fees",
                "attendace_fees",
    ...
    public function show_fee($id, $fee_id)
    {
        ...
        if ($fee["category"]["category_name"] == "Attendance Fees") {
            $attendance_fees = AttendanceFee::where(
                "fee_id",
                $fee["id"]
            )->get();
            // dd($attendace_fees);
            // $attendace_fees[array]['attendanceRecord']['event']['event_name'];
            $groupedFees = $attendance_fees->groupBy(
                fn($fee) => $fee->attendanceRecord->event->event_name
            );
            // ->map(function ($fees) {
            //     return [
            //         'fees' => $fees,
            ...
            // dd($groupedFees);
            return view(
                "student_list.actions.view_student_attendance_fees",
                compact("student", "groupedFees", "fee")
            );
        }

        return view(
            "student_list.actions.view_student_fees",
            compact("student", "fee")
        );
        // dd("Not Attendace");
    }
    ...
    public function show_payment($id, $payment_id)
    {
        ...
        // dd($payment);
        return view(
            "student_list.actions.view_student_payment",
    ...
    public function show_payment_submission($id, $payment_submission_id)
    {
        ...
        $payment_submission = PaymentSubmission::findOrFail(
            $payment_submission_id
        );

        // dd($payment_submission);
        return view(
            "student_list.actions.view_student_payment_submission",
            compact("student", "payment_submission")
        );
    }
}
