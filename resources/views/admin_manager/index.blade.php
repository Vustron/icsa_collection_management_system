...
<x-layout>
    <div id="messages">
        @if (session('deleted'))
            <div class="border border-gray-400 text-gray-900 p-3 mb-2">
                {{ session('deleted') }}
            </div>
        @endif
        @if (session('success'))
            <div class="border border-green-400 text-green-600 p-3 mb-2">
                {{ session('success') }}
            </div>
        @endif
        @if (session('error'))
        ...
        @foreach ($users as $user)
            @if ($user['id'] != auth()->user()['id'])
                <tr class="hover:bg-violet-100 last:border-violet-700 even:bg-gray-100">
                    <td class="px-4 py-1 text-center border border-gray-300">{{ $user['id'] }}</td>
                    ...
                    <td class="px-4 py-2 border border-gray-300">
                        ...
                        <div class=" border-none {!! $authorized ? 'text-green-700' : 'text-red-700' !!} py-1 px-1 rounded-sm capitalize">
                    ...
                    <td class="px-2 py-1 border border-gray-300">
                        ...
                        <div
                            class="border-none text-white text-center py-1 px-1 rounded-sm 
                            capitalize {!! $user['status'] == 'active' ? 'bg-green-500' : 'bg-red-500' !!}">
                            {{ $user['status'] }}
                        </div>
                    </td>
                    <td class="px-4 py-2 border border-gray-300">
                        <div class="flex justify-center items-center gap-2">
                            <x-bi-eye-fill class="size-5 text-green-500 hover:text-green-700 cursor-pointer view-user"
                                data-user="{{ json_encode($user) }}" />
                            <x-bx-pencil class="size-5 text-orange-500 hover:text-orange-700 cursor-pointer edit-user"
                                data-user="{{ json_encode($user) }}" />
                            <x-bi-trash
                                class="size-5
                                text-red-500 hover:text-red-700 cursor-pointer delete-user"
    ...
    <div class="flex justify-center mt-3 md:mt-0 gap-2 md:justify-between items-center px-2 flex-wrap-reverse">
        <div>Showing 1 to n of n entries</div>
        <div class="flex items-center space-x-1 text-blue-600">
            <button class="px-2 py-1 border rounded-md hover:bg-gray-200">Previous</button>
            <button class="px-2 py-1 border rounded-md hover:bg-gray-200">1</button>
            <span class="px-2">...</span>
            <button class="px-2 py-1 border rounded-md hover:bg-gray-200">5</button>
            <button class="px-2 py-1 border rounded-md bg-[#9334ea] text-white">6</button>
            <button class="px-2 py-1 border rounded-md hover:bg-gray-200">7</button>
            <span class="px-2">...</span>
            <button class="px-2 py-1 border rounded-md hover:bg-gray-200">156</button>
            <button class="px-2 py-1 border rounded-md hover:bg-gray-200">Next</button>
        </div>
    </div>
