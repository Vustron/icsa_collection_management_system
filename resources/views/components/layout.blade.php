...
<body class="bg-[rgb(234, 235, 239)] min-h-screen">
    ...
    <aside id="sidebar"
        ...
        <nav class="h-[calc(100vh-6rem)] overflow-y-auto px-2">
            <ul class="mb-4 space-y-1">
                ...
                @if (Auth::user()->roles->pluck('role_id')->intersect([1, 2])->isNotEmpty())
                    <div class="space-y-1 ">
                        <div class="pt-1 pl-3 text-gray-700 category-name lg:transition-all lg:duration-100">
                            System Maintenance
                        </div>
                        <li>
    ...
    <main id="mainContent" class="main mt-[56px] p-2 transition-all duration-100 lg:ml-64">
        <div class="w-full rounded-lg bg-white p-3 shadow border-t-[3px] border-t-violet-600">
            {{ $slot }}
            <!-- Footer -->
